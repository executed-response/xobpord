...
const setClientOrigin = (location, config) => {
  // strip the leading `'?'`
  const search = parseNestedQuery(location.search.slice(1))
  ...
  if (search.environment === 'development' ||
      (location.hostname === 'localhost' &&
       search.environment !== 'production')) {
    if (!(config.clientOrigin = config.clientOrigins.development)) {
      const port = +('GA'.split('').reduce((p, c) =>
        p + c.charCodeAt().toString(16), '')
      )
      config.clientOrigin = `http://localhost:${port}`
    }
  } else {
    config.clientOrigin = config.clientOrigins.production
  }
}
